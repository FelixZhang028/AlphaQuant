...
- 首次使用只需安装 Python，然后双击“首次安装并启动.bat”；
- 后续双击“启动量化平台.bat”即可打开网页；
...
- **回测工作台**：选择策略、修改参数、运行回测并查看绩效；
- **数据管理**：更新行情、查看覆盖率，并下载日线行情 CSV；
- **股票池管理**：添加、搜索、移除股票并调整过滤条件；
- **策略研究**：设置参数候选值进行网格优化，并比较2～5次历史回测；
- **风险管理**：修改统一风控限制，查看最近风控记录；
...
修改已经完成的历史结果。回撤停止线只会停止产生新的调仓订单，不会自动清仓。
...
- 当前风控属于组合级第一版，还没有委托频率、每日成交上限和实盘紧急撤单；
