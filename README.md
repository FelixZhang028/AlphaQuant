...
- 提供 LLM 多智能体策略插件（分析师团队、多空辩论、风控和组合经理审批，见 `docs/llm_agent_strategy.md`）；
- 在"智能体分析台"页面对单只股票运行 LLM 多智能体研究并展示完整中间产物。
...
LLM 多智能体功能新增了 `pydantic`、`requests`、`rich` 依赖，已包含在项目依赖中，
重新执行安装命令即可。
...
src/quant_platform/
  ...
  accounts/                      账户和持仓
  agents_bridge/                 与 trading_agents 决策流水线的桥接层
  ...
  universe/                      股票池
  web/                           Streamlit 页面
src/trading_agents/              多智能体决策框架（来自 EasyQuant）
tests/                           单元和集成测试
runtime/                         本地数据与运行结果
```
