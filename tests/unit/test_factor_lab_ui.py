...
def test_library_filters_and_handoff(tmp_path, monkeypatch):
    ...
    # AppTest has no row-selection adapter; inject the native selection event
    # state via the conftest-patched _test_selection attribute (string_value).
    assert not any(button.key == "library_to_eval" for button in app.button)
    ...
    app.button(key="library_close").click().run()
    assert not any(button.key == "library_to_eval" for button in app.button)
