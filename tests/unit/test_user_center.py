...
def test_account_center_sections_and_logout(monkeypatch, tmp_path):
    ...
    app.run().switch_page("app_pages/16_user_center.py").run()
...
@pytest.mark.parametrize(
    "origin,title",
    [
        ("app_pages/14_settings.py", "设置"),
        ("app_pages/0_strategy_hub.py", "策略工作室"),
    ],
)
def test_account_returns_to_origin_after_section_changes(origin, title, monkeypatch, tmp_path):
    ...
    app.switch_page("app_pages/16_user_center.py").run()
