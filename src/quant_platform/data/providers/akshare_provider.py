...
class AkShareDataProvider(DataProvider):
    ...
    def get_trade_calendar(self, start_date: date, end_date: date) -> pd.DataFrame:
        frame = self._ak.tool_trade_date_hist_sina().rename(columns={"trade_date": "cal_date"})
    ...
    def get_adjustment_factors(
        ...
        raise DataCapabilityNotSupported("AkShare adapter does not expose stable adj_factor data")

    def get_price_limits(self, trade_date: date, symbols: list[str] | None = None) -> pd.DataFrame:
        raise DataCapabilityNotSupported("AkShare adapter does not expose canonical price limits")
