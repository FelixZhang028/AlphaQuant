...
from quant_platform.web.data_job_panel import render_jobs
...
jobs = DataJobs(
    Path(str(service.app.get("app", {}).get("runtime_dir", "runtime"))) / "data_jobs",
...
if section == "全市场回填":
    ...
    render_jobs(jobs, history=True)
else:
    if jobs.active():
        st.info("全市场任务正在运行。请在全市场回填中查看进度或停止任务。")
    ...
    with st.expander("更新数据", expanded=True):
        with st.form("data_update_form"):
            ...
            submitted = st.form_submit_button(
                "开始更新", type="primary", disabled=bool(jobs.active())
            )
    ...
    with market_tab:
        ...
        if daily_bars.empty:
            ...
            selected_bars = daily_bars[
                ...
                & daily_bars["trade_date"].between(
                    pd.Timestamp(export_start), pd.Timestamp(export_end)
                )
