...
/* ============ Hero（DeepSeek Harness 着陆页风格：深蓝丝绸光带 + 终端卡） ============ */
.aq-hero {
    position: relative;
    text-align: left;
...
.aq-term {
    ...
    overflow: hidden;
    animation:
        aqFadeUp 0.6s ease-out 0.3s both,
        aqFloatY 7s ease-in-out 0.6s infinite;
}
@keyframes aqFloatY {
    0%, 100% { transform: translateY(0); }
...
.st-key-aq_stickybar {
    ...
    top: 1.5rem;                     /* 下移更多，避免贴近浏览器/应用顶部边缘 */
    left: 0;
    right: 0;
    z-index: 1001;
    width: 100%;
...
def topbar_html(wordmark: str = "AlphaQuant", pills: tuple[str, ...] = ()) -> str:
...
def render_topbar(wordmark: str = "AlphaQuant", pills: tuple[str, ...] = ()) -> None:
...
def render_hero(
  ...
  <div class="aq-term">
    ...
    <div class="aq-term-body">{lines}<span class="aq-cursor"></span></div>
  </div>"""
    st.markdown(
        _LANDING_CSS
        + f"""
