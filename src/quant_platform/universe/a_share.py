...
class AShareUniverse(Universe):
    ...
    def select(self, trade_date: date, history: pd.DataFrame) -> list[str]:
        ...
        available = history[
            history["symbol"].isin(self.config.symbols)
            & (history["trade_date"] <= cutoff)
        ].sort_values(["symbol", "trade_date"])
        eligible: list[str] = []
        required_days = max(
            self.config.minimum_history_days, self.config.minimum_listing_days
        )
        for symbol, group in available.groupby("symbol", observed=True):
            ...
            if str(latest.get("quality_status", "OK")) not in {"OK", "UNKNOWN_STATUS"}:
                continue
            if self.config.exclude_suspended and bool(
                latest.get("is_suspended", False)
            ):
                continue
